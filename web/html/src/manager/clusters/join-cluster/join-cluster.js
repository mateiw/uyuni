...
const JoinCluster = (props: Props) => {
    ...
    const scheduleJoin = (earliest: Date, actionChain: ?string): Promise<any> => {
        if (nodeToJoin && joinConfig && providerConfig) {
            return scheduleJoinNode(props.cluster.id, nodeToJoin.id, providerConfig, joinConfig, earliest, actionChain);
        }
                    ...
                    <Route path="select-node">
                        {({goTo, back}) =>
                            <SelectServer title={t("Select system to join")}
                                ...
                                onNext={(node) => {setNodeToJoin(node); goTo("provider-config")}}/>}                             
                    </Route>
                    <Route path="provider-config">
                        {({goTo, back}) =>
                            <FormulaConfig values={providerConfig}
                                ...
                                formula="config"
                                onNext={(formulaValues) => {setProviderConfig(formulaValues); goTo("join-config");}}
                                onPrev={back} />}
                    </Route>
